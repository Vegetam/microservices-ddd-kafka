...
export class EventConsumer implements OnApplicationBootstrap {
  ...
  async onApplicationBootstrap(): Promise<void> {
    await this.kafka.subscribe('order.confirmed', 'notif-order-confirmed', async (p) =>
      this.confirmedHandler.handle(p as unknown as OrderConfirmedPayload),
    );

    await this.kafka.subscribe('order.cancelled', 'notif-order-cancelled', async (p) =>
      this.cancelledHandler.handle(p as unknown as OrderCancelledPayload),
    );
